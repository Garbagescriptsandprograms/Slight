...
class Aimbot:
    ...
    def __init__(self, box_constant = 416, collect_data = False, mouse_delay = 0.0001, debug = False): # box_constant if you wanna do oold do 416 dont forget
        ...
        print("Loading Aimassist")
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='lib/best.pt', force_reload = True)
        if torch.cuda.is_available():
            print(colored("YOU RICH SON A BITCH YOU GOT CUDA/NVIDIA GPU LUCKY BASTARD", "green"))
        else:
            print(colored("[IMPORTANT] Cuda is not available it wont be as powerful or fast still good tbh", "red"))
            print(colored("[IMPORTANT] Check your pytorch unless you got amd gpu if you do im sorry i feel bad", "red"))
        ...
        print("\n[INFORMATION] Press 'F1' to toggle the aim assist \n[INFORMATION] Press 'F2' to close it")
    ...
    def start(self):
        print("Starting Screen Capture") #Its beginning screen capture man dont forget
        ...
        while True:
            ...
            if len(results.xyxy[0]) != 0: #player detected
                ...
                if closest_detection: #if valid detection exists
                    ...
                    if Aimbot.is_target_locked(absolute_head_X, absolute_head_Y):
                        cv2.putText(frame, "Locked on", (x1 + 40, y1), cv2.FONT_HERSHEY_DUPLEX, 0.5, (115, 244, 113), 2) #draw the confidence labels on the bounding boxes
                    else:
            ...
            if self.collect_data and time.perf_counter() - collect_pause > 1 and Aimbot.is_targeted() and Aimbot.is_aimbot_enabled() and not player_in_frame: #screenshots can only be taken every 1 second
             ...
             # if no work put 'cv2.putText(frame, f"Fps: {int(1/(time.perf_counter() - start_time))}", (5, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (113, 116, 244), 2)'
             # if no work put 'cv2.imshow("Propain Vision", frame)'

            if cv2.waitKey(1) & 0xFF == ord('0'):
                break

    def clean_up():
        print("\nHey you closed it why")
        Aimbot.screen.close()
        os._exit(0)

if __name__ == "__main__": print("Run propain.py not this idiot")
